...
pub const retro_language_RETRO_LANGUAGE_LAST: retro_language = 28;
...
pub type retro_core_options_update_display_callback_t = ::std::option::Option<unsafe extern "C" fn() -> bool>;
#[repr(C)]
#[derive(Debug, Copy, Clone)]
...
lazy_static! {
    static ref LOG_CB: Arc<Mutex<retro_log_printf_t>> = Arc::new(Mutex::new(None));
    static ref ENVIRONMENT_CB: Arc<Mutex<retro_environment_t>> = Arc::new(Mutex::new(None));
    static ref VIDEO_CB: Arc<Mutex<retro_video_refresh_t>> = Arc::new(Mutex::new(None));
    static ref AUDIO_CB: Arc<Mutex<retro_audio_sample_t>> = Arc::new(Mutex::new(None));
    static ref AUDIO_BATCH_CB: Arc<Mutex<retro_audio_sample_batch_t>> = Arc::new(Mutex::new(None));
    static ref INPUT_POLL_CB: Arc<Mutex<retro_input_poll_t>> = Arc::new(Mutex::new(None));
...
#[no_mangle]
pub extern "C" fn retro_set_environment(arg1: retro_environment_t) {
    let cb = arg1.expect("retro_environment_t cb isn't valid");
    {
        let mut environment_cb = ENVIRONMENT_CB.lock().expect("Couldn't lock environment_cb");
...
#[no_mangle]
pub extern "C" fn retro_set_video_refresh(arg1: retro_video_refresh_t) {
    let mut video_cb = VIDEO_CB.lock().expect("Couldn't lock video_cb");
    *video_cb = Some(arg1.expect("retro_video_refresh isn't valid"));
}

#[no_mangle]
pub extern "C" fn retro_set_audio_sample(arg1: retro_audio_sample_t) {
    let mut audio_cb = AUDIO_CB.lock().expect("Couldn't lock audio_cb");
    *audio_cb = Some(arg1.expect("retro_audio_sample isn't valid"));
}

#[no_mangle]
pub extern "C" fn retro_set_audio_sample_batch(arg1: retro_audio_sample_batch_t) {
    let mut audio_batch_cb = AUDIO_BATCH_CB.lock().expect("Couldn't lock audio_batch_cb");
    *audio_batch_cb = Some(arg1.expect("retro_audio_sample_batch isn't valid"));
}

#[no_mangle]
pub extern "C" fn retro_set_input_poll(arg1: retro_input_poll_t) {
    let mut input_poll_cb = INPUT_POLL_CB.lock().expect("Couldn't lock input_poll_cb");
    *input_poll_cb = Some(arg1.expect("retro_input_poll isn't valid"));
}

#[no_mangle]
pub extern "C" fn retro_set_input_state(arg1: retro_input_state_t) {
    let mut input_state_cb = INPUT_STATE_CB.lock().expect("Couldn't lock input_state_cb");
    *input_state_cb = Some(arg1.expect("retro_input_state isn't valid"));
}

#[no_mangle]
pub extern "C" fn retro_init() {
    let mut gameboy = GAME_BOY.lock().expect("Couldn't lock game_boy");
    *gameboy = Some(Gameboy::new());
}
...
#[no_mangle]
pub extern "C" fn retro_api_version() -> ::std::os::raw::c_uint { RETRO_API_VERSION }

#[no_mangle]
pub extern "C" fn retro_get_system_info(info: *mut retro_system_info) {
    let mut retro_system_info_ref = unsafe { info.as_mut() }.expect("Invalid info:retro_system_info");
    let library_name = CString::new("RPGBE").expect("Cstring::new RPGBE failed");
    let library_version = CString::new("0.1").expect("Cstring::new 0.1 failed");
    let valid_extensions = CString::new("gb").expect("Cstring::new empty failed");
    retro_system_info_ref.library_name = library_name.as_ptr();
    retro_system_info_ref.library_version = library_version.as_ptr();
    retro_system_info_ref.valid_extensions = valid_extensions.as_ptr();
    retro_system_info_ref.need_fullpath = false;
    std::mem::forget(library_name);
    std::mem::forget(library_version);
    std::mem::forget(valid_extensions);
}

#[no_mangle]
pub extern "C" fn retro_get_system_av_info(info: *mut retro_system_av_info) {
    let mut retro_system_av_info_ref = unsafe { info.as_mut() }.expect("Invalid info:retro_system_av_info");
    retro_system_av_info_ref.geometry.base_width = WIDTH as u32;
    retro_system_av_info_ref.geometry.base_height = HEIGHT as u32;
    retro_system_av_info_ref.geometry.max_width = WIDTH as u32;
    retro_system_av_info_ref.geometry.max_height = HEIGHT as u32;
...
#[no_mangle]
pub extern "C" fn retro_set_controller_port_device(_port: ::std::os::raw::c_uint, _device: ::std::os::raw::c_uint,) { }
...
pub extern "C" fn retro_run() {
    ...
    let screen_data = match gameboy.deref() {
        Some(t) => { t.screen_data },
        _ => [0; SCREEN_DATA_SIZE as usize],
    };
...
#[no_mangle]
pub extern "C" fn retro_serialize(_data: *mut ::std::os::raw::c_void, _size: size_t) -> bool { false }

#[no_mangle]
pub extern "C" fn retro_unserialize(_data: *const ::std::os::raw::c_void, _size: size_t) -> bool { false }

#[no_mangle]
pub extern "C" fn retro_cheat_reset() { }

#[no_mangle]
pub extern "C" fn retro_cheat_set(_index: ::std::os::raw::c_uint, _enabled: bool, _code: *const ::std::os::raw::c_char,) { }

#[no_mangle]
pub extern "C" fn retro_load_game(game: *const retro_game_info) -> bool {
    let retro_game_info_ref = unsafe { game.as_ref() }.expect("Invalid game:retro_game_info");
    let mut gameboy = GAME_BOY.lock().expect("Couldn't lock game_boy");

    let path_cstr = unsafe { CStr::from_ptr(retro_game_info_ref.path) };
    let pathbuf = PathBuf::from(path_cstr.to_string_lossy().to_string());
    match gameboy.deref_mut() {
        Some(t) => {
            t.memory.load_cartridge_from_path(pathbuf);
            true
        },
        _ => false,
    }
...
#[no_mangle]
pub extern "C" fn retro_load_game_special(_game_type: ::std::os::raw::c_uint, info: *const retro_game_info, _num_info: size_t,) -> bool { retro_load_game(info) }

#[no_mangle]
pub extern "C" fn retro_unload_game() { }

#[no_mangle]
pub extern "C" fn retro_get_region() -> ::std::os::raw::c_uint { RETRO_REGION_NTSC }

#[no_mangle]
pub extern "C" fn retro_get_memory_data(_id: ::std::os::raw::c_uint) -> *mut ::std::os::raw::c_void { std::ptr::null_mut() }

#[no_mangle]
pub extern "C" fn retro_get_memory_size(_id: ::std::os::raw::c_uint) -> size_t { 0 }